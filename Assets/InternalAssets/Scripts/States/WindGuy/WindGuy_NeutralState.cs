...
public class WindGuy_NeutralState : State
{
    public WindGuy_NeutralState(BeingBehavior sub_, Interactiblebutton interactibleButtonEnum_, float interactionRadius_, NameState nameState_) : base(sub_, interactibleButtonEnum_, interactionRadius_, nameState_) {
    }
    ...
    public override void BInteraction() {
        ...
        if (player.nameState.Equals(NameState.Fire)) {
            ...
            sub.SetState(new WindGuy_FireState(sub, interactibleButtonEnum, interactionRadius, nameState));
        }
        if (player.nameState.Equals(NameState.Water)) {
            nameState = NameState.Water;
            player.nameState = NameState.Neutral;
            player.SetState(new PlayerEmpty(player, player.interactiblebuttonenum, player.interactionradius, player.nameState));
            sub.SetState(new WindGuy_WaterState(sub, interactibleButtonEnum, interactionRadius, nameState));
        }
        if (player.nameState.Equals(NameState.Wind)) {
            nameState = NameState.Wind;
            player.nameState = NameState.Neutral;
            player.SetState(new PlayerEmpty(player, player.interactiblebuttonenum, player.interactionradius, player.nameState));
            sub.SetState(new WindGuy_WindState(sub, interactibleButtonEnum, interactionRadius, nameState));
        }
        if (player.nameState.Equals(NameState.Earth)) {
            nameState = NameState.Earth;
            player.nameState = NameState.Neutral;
            player.SetState(new PlayerEmpty(player, player.interactiblebuttonenum, player.interactionradius, player.nameState));
            sub.SetState(new WindGuy_EarthState(sub, interactibleButtonEnum, interactionRadius, nameState));
        }
